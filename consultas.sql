...
SELECT COUNT(*) usuario FROM Usuarios WHERE NOT activo or saldo <= 0;
...
SELECT usuario, COUNT(*) FROM Usuarios WHERE nivel=2;
...
SELECT email FROM Usuarios WHERE email <> '%yahoo.com';
...
SELECT nombre, telefono FROM Usuarios WHERE compania NOT IN( 'MOTOROLA' , 'NOKIA');
